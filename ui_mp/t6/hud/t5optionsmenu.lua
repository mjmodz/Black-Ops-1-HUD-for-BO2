...
CoD.T5OptionsMenu = CoD.T5OptionsMenu or {}
CoD.T5PauseMenu = CoD.T5PauseMenu or {}
CoD.T5PauseMenu.StoredHudRefs = CoD.T5PauseMenu.StoredHudRefs or {}

local OPTION_BUTTON_TEXT_LEFT = 79
local OPTION_BUTTON_TEXT_RIGHT = 370

local OPTION_TITLE_TEXT_LEFT = OPTION_BUTTON_TEXT_LEFT + 10
local OPTION_TITLE_TEXT_RIGHT = OPTION_TITLE_TEXT_LEFT + 132
...
local OPTION_BUTTONS = {
	{ text = "CONTROLS", action = "options_open_controls", left = OPTION_BUTTON_TEXT_LEFT, right = OPTION_BUTTON_TEXT_RIGHT, top = 114, bottom = 136, highlightTop = 113, highlightBottom = 137 },
	{ text = "SETTINGS", action = "options_open_settings", left = OPTION_BUTTON_TEXT_LEFT, right = OPTION_BUTTON_TEXT_RIGHT, top = 142, bottom = 164, highlightTop = 141, highlightBottom = 165 }
}
...
local function OpenChild(menu, controller, menuName)
	...
	if nextMenu ~= nil and nextMenu.setPreviousMenu ~= nil then
		nextMenu:setPreviousMenu("T5OptionsMenu")
	end
...
CoD.T5OptionsMenu.Back = function(menu, event)
	local controller = 0
...
CoD.T5OptionsMenu.OpenControlsPressed = function(menu, event)
	if event ~= nil and event.controller ~= nil then
		return OpenChild(menu, event.controller, "OptionsControlsMenu")
	end

	return true
end

CoD.T5OptionsMenu.OpenSettingsPressed = function(menu, event)
	if event ~= nil and event.controller ~= nil then
		return OpenChild(menu, event.controller, "OptionsSettingsMenu")
	end
...
local function AddButton(parent, info)
	...
	button.id = "options_" .. info.action
...
LUI.createMenu.T5OptionsMenu = function(controller)
	local menu = CoD.Menu.NewFromState("T5OptionsMenu", {
		leftAnchor = true,
		rightAnchor = true,
		left = 0,
		right = 0,
	...
	if CoD.T5PauseMenu.StoredHudRefs ~= nil then
		...
		if menu.gameHud ~= nil then
			menu.gameHud.t5OptionsMenu = menu
		end
	end
	...
	menu:registerEventHandler("button_prompt_back", CoD.T5OptionsMenu.Back)
	menu:registerEventHandler("button_prompt_start", CoD.T5OptionsMenu.Back)
	menu:registerEventHandler("close_all_ingame_menus", CoD.T5OptionsMenu.Back)
	menu:registerEventHandler("close_ingame_menu", CoD.T5OptionsMenu.Back)
	menu:registerEventHandler("options_open_controls", CoD.T5OptionsMenu.OpenControlsPressed)
	menu:registerEventHandler("options_open_settings", CoD.T5OptionsMenu.OpenSettingsPressed)
	menu:registerEventHandler("options_enable_back_prompts", SetupBack)
	menu:registerEventHandler("options_force_cursor_visible", KeepCursor)
	menu:registerEventHandler("options_unblur_world", KeepBlur)

	AddBox(menu, 0, 1280, 0, 720, 0, 0, 0, 0.53)
	AddEdgeFade(menu)
	AddButtonBacks(menu, OPTION_BUTTONS)

	menu.highlight = AddBox(menu, BUTTON_BACKING_LEFT, BUTTON_BACKING_RIGHT, 113, 137, 1, 1, 1, 0)

	AddLabel(menu, OPTION_TITLE_TEXT_LEFT, OPTION_TITLE_TEXT_RIGHT, 63, 105, "OPTIONS", CoD.fonts.Condensed, 1.10)

	menu.zmbIcon = AddZmbIcon(menu)

	menu.buttons = {}
	for i = 1, #OPTION_BUTTONS, 1 do
		menu.buttons[i] = AddButton(menu, OPTION_BUTTONS[i])
	end

	WireButtons(menu.buttons)

	menu:addElement(LUI.UITimer.new(1, "options_unblur_world", true, menu))
	menu:addElement(LUI.UITimer.new(1, "options_force_cursor_visible", true, menu))
	menu:addElement(LUI.UITimer.new(75, "options_enable_back_prompts", true, menu))
