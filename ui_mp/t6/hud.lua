...
require("T6.HUD.t5pausemenu")
require("T6.HUD.t5optionsmenu")
require("T6.HUD.t5confirmmenu")
require("T6.HUD.Killcam")
require("T6.HUD.ManageSegments")
require("T6.HUD.Migration")
require("T6.HUD.ScoreFeed")
require("T6.HUD.t5scoreboard")
...
if CoD.isZombie == true then
	...
	require("T6.Zombie.t5hudcompetitivescoreboardzombie")
	require("T6.Zombie.t5huddpadareazombie")
	require("T6.Zombie.t5hudperkszombie")
	require("T6.Zombie.t5hudpowerupszombie")
	require("T6.Zombie.t5hudroundstatuszombie")
	require("T6.Zombie.HudTimerZombie")
	require("T6.Zombie.OtherAmmoCounters")
	require("T6.Zombie.t5ammocounter")
else
	require("T6.VcsMenu")
...
LUI.createMenu.HUD = function(LocalClientIndex)
	...
	if CoD.isPC then
		...
		if LUI.roots.UIRootFull.mouseCursor == nil then
			LUI.roots.UIRootFull.mouseCursor = LUI.UIMouseCursor.new({
				leftAnchor = true,
				rightAnchor = false,
				left = 0,
				right = 64,
			...
			LUI.roots.UIRootFull:addElement(LUI.roots.UIRootFull.mouseCursor)
		end
	end
...
function HUD_FirstSnapshot_Zombie(HUDWidget, ClientInstance)
	...
	if CoD.Zombie.IsDLCMap(CoD.Zombie.DLC2Maps) then
		...
		require("T6.Zombie.t5hudafterlifedisplay")
		local afterlifeArea = LUI.createMenu.AfterlifeArea(ClientInstance.controller)
		afterlifeArea:setUseGameTime(true)
		Widget:addElement(afterlifeArea)
	elseif CoD.Zombie.IsDLCMap(CoD.Zombie.DLC4Maps) then
		if CoD.Zombie.GAMETYPE_ZCLASSIC == Dvar.ui_gametype:get() then
			require("T6.Zombie.t5hudcraftablestombzombie")
	...
	local scoreboard = LUI.createMenu.CompetitiveScoreboard(ClientInstance.controller)
	scoreboard:setUseGameTime(true)
	Widget:addElement(scoreboard)

	local t5AmmoCounter = LUI.createMenu.T5AmmoCounter(ClientInstance.controller)
	t5AmmoCounter:setUseGameTime(true)
	HUDWidget.t5AmmoCounter = t5AmmoCounter
	Widget:addElement(t5AmmoCounter)

	if CoD.Zombie.GAMETYPE_ZCLEANSED == Dvar.ui_gametype:get() then
		local timerArea = LUI.createMenu.TimerAreaZM(ClientInstance.controller)
		timerArea:setUseGameTime(true)
		Widget:addElement(timerArea)
	else
		local roundStatus = LUI.createMenu.RoundStatus(ClientInstance.controller)
		roundStatus:setUseGameTime(true)
		Widget:addElement(roundStatus)
	end
...
HUD_OpenIngameMenu = function(HUDWidget, ClientInstance)
	...
	HUDWidget:dispatchEventToChildren({
		name = "t5_hud_force_hide",
		controller = ClientInstance.controller,
	})
	HUDWidget:dispatchEventToChildren(ClientInstance)
	if CoD.isZombie == true and LUI.createMenu.T5PauseMenu ~= nil then
		if HUDWidget.t5PauseMenu ~= nil then
			HUDWidget.t5PauseMenu:processEvent({
				name = "t5_internal_close",
				controller = ClientInstance.controller,
			})
			return
		end

		if CoD.T5PauseMenu ~= nil then
			CoD.T5PauseMenu.StoredHudRefs = CoD.T5PauseMenu.StoredHudRefs or {}
			CoD.T5PauseMenu.StoredHudRefs[ClientInstance.controller + 1] = HUDWidget
		end

		HUDWidget.t5PauseMenu = LUI.createMenu.T5PauseMenu(ClientInstance.controller)
		HUDWidget.t5PauseMenu.gameHud = HUDWidget
		HUDWidget.t5PauseMenu.mj_gameHud = HUDWidget
		LUI.roots.UIRootFull:addElement(HUDWidget.t5PauseMenu)
		HUDWidget:setAlpha(0)
		Engine.BlurWorld(ClientInstance.controller, 2)
		Engine.LockInput(ClientInstance.controller, true)
		Engine.SetUIActive(ClientInstance.controller, true)
		if CoD.isPC then
	...
	local menu = LUI.createMenu[ClientInstance.menuName]
	if menu ~= nil then
