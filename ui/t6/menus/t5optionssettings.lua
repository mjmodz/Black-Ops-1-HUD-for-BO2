...
local function T5CleanupVideoRestartMenuStack(Controller)
	...
	if CoD.T5PauseMenu ~= nil and CoD.T5PauseMenu.StoredHudRefs ~= nil then
		HudRef = CoD.T5PauseMenu.StoredHudRefs[Controller + 1]
	end

	if HudRef ~= nil then
		if HudRef.t5PauseMenu ~= nil then
			HudRef.t5PauseMenu:close()
			HudRef.t5PauseMenu = nil
		end

		if HudRef.t5OptionsMenu ~= nil then
			HudRef.t5OptionsMenu:close()
	...
	if CoD.isPC then
		...
		if LUI.roots ~= nil and LUI.roots.UIRootFull ~= nil and LUI.roots.UIRootFull.mouseCursor ~= nil then
...
CoD.OptionsSettings.LeaveApplyPopup_DeclineApply = function (f2_arg0, ClientInstance)
	if UIExpression.IsInGame() == 1 and LUI.createMenu.T5OptionsMenu ~= nil then
		f2_arg0:setPreviousMenu("T5OptionsMenu")
	else
...
CoD.OptionsSettings.ApplyPopup_ApplyChanges = function (f4_arg0, ClientInstance)
	...
	local NeedsVideoRestartCleanup = CoD.OptionsSettings.NeedVidRestart == true

	if NeedsVideoRestartCleanup then
		T5CleanupVideoRestartMenuStack(Controller)
	end
...
CoD.OptionsSettings.Back = function (f5_arg0, ClientInstance)
	if CoD.OptionsSettings.NeedVidRestart or CoD.OptionsSettings.NeedPicmip or CoD.OptionsSettings.NeedSndRestart then
		...
		-- Use the same safe back path as the main Options menu. This keeps
		-- the custom T5 pause menu from falling back to the old ZRM previous-menu chain.
		CoD.Options.Back(f5_arg0, ClientInstance)
	end
end
...
CoD.OptionsSettings.DefaultPopup_RestoreDefaultSettings = function (f17_arg0, ClientInstance)
	...
	T5CleanupVideoRestartMenuStack(Controller)
	CoD.OptionsSettings.ResetDvars(Controller)
	Engine.ResetHardwareProfileSettings(Controller)
	Engine.Exec(Controller, "r_applyPicmip")
	Engine.Exec(Controller, "vid_restart")
	Engine.Exec(Controller, "snd_restart")
	CoD.OptionsSettings.ResetSoundToDefault(Controller)
	CoD.OptionsSettings.ResetGameToDefault(Controller)
	Engine.SaveHardwareProfile()
...
LUI.createMenu.OptionsSettingsMenu = function (LocalClientIndex)
	...
	if InGame and LUI.createMenu.T5OptionsMenu ~= nil then
		OptionsSettingsWidget:setPreviousMenu("T5OptionsMenu")
	else
